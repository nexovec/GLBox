...
New_Vbo_Example::New_Vbo_Example()
{
    ...
    this->color_loc = glGetAttribLocation(this->program, "pos");
    ...
    glBufferData(GL_ARRAY_BUFFER, 9 * sizeof(float), this->data_containers.positions, GL_DYNAMIC_DRAW);

    glBindBuffer(GL_ARRAY_BUFFER, this->vbo_indices.colors);
    glBufferData(GL_ARRAY_BUFFER, 9 * sizeof(float), this->data_containers.colors, GL_DYNAMIC_DRAW);
}
